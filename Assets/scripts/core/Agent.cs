...
namespace RVO
{
    ...
    internal abstract class Agent : SuperAgent
    {
        ...
        public float sharingEffort;


        /* New RVOAgent :
         * New characteristics added to the RVO Agent, these characteristics define an agent's profile
         * Fast : if (Fast == true), the agent is fast ; else, the agent is slow
         * Watchful : if (Watchful == true), the agent is attentive (walking properly) ; else, the agent is distracted (walking while looking at its phone)
         * Man : if (Man == true), the agent is a man ; else, it is a woman
         * #Cleo */
        internal bool Fast;
        internal bool Watchful;
        internal bool Man;
         ...
         */
        public abstract void interactWith(Agent agent);
        ...
        */
        public void interactWithAgents()
        {
            ...
            for (int i = 0; i < groups.Count; ++i)
            {
                ...
                {
                    Agent groupRepresentation2 = (Agent)groupRepresentation; //the groupRepresentation function is supposed to use a Super Agent, but we force it to use an Agent #Cleo
                    interactWith(groupRepresentation2);
                }
